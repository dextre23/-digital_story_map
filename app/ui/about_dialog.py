from PySide6.QtCore import Qt
from PySide6.QtWidgets import QDialog, QLabel, QPushButton, QScrollArea, QVBoxLayout

from app.i18n import t


class AboutDialog(QDialog):
    def __init__(self, parent=None, locale: str = "tr") -> None:
        super().__init__(parent)
        self._locale = locale
        self.resize(540, 600)
        self.setModal(True)

        self._head = QLabel()
        self._head.setObjectName("aboutHead")

        self._sub = QLabel()
        self._sub.setObjectName("aboutSub")

        self._body = QLabel()
        self._body.setObjectName("aboutBody")
        self._body.setTextFormat(Qt.TextFormat.RichText)
        self._body.setWordWrap(True)
        self._body.setAlignment(Qt.AlignmentFlag.AlignTop)

        self._techspec_title = QLabel()
        self._techspec_title.setObjectName("aboutTechSpecTitle")
        self._techspec_title.setWordWrap(True)

        self._techspec = QLabel()
        self._techspec.setObjectName("aboutTechSpec")
        self._techspec.setTextFormat(Qt.TextFormat.RichText)
        self._techspec.setWordWrap(True)
        self._techspec.setAlignment(Qt.AlignmentFlag.AlignTop)

        self._close_btn = QPushButton()
        self._close_btn.clicked.connect(self.accept)

        layout = QVBoxLayout(self)
        layout.setSpacing(14)
        layout.addWidget(self._head)
        layout.addWidget(self._sub)
        layout.addWidget(self._body)
        layout.addSpacing(8)
        layout.addWidget(self._techspec_title)
        layout.addWidget(self._techspec, stretch=1)
        layout.addWidget(self._close_btn, alignment=Qt.AlignmentFlag.AlignRight)

        self._apply_texts()

    def _apply_texts(self) -> None:
        self.setWindowTitle(t(self._locale, "about_window"))
        self._head.setText(t(self._locale, "about_head"))
        self._sub.setText(t(self._locale, "about_sub"))
        self._body.setText(str(t(self._locale, "about_body")))
        self._techspec_title.setText(str(t(self._locale, "about_techspec_title")))
        self._techspec.setText(str(t(self._locale, "about_techspec")))
        self._close_btn.setText(t(self._locale, "close"))
